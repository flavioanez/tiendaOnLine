...
@Component({
    selector: 'app-cart-wholesaler',
    templateUrl: './cart-wholesaler.component.html',
    styleUrls: ['./cart-wholesaler.component.css']
})
export class CartWholesalerComponent implements OnInit {
    ...
    deleteProduct(code: string) {
        ...
        this._storeServiceMayoristas.deleteProductMayorista(code, this.idUser).then(data => {
        }, err => {
            console.log(err);
        });
    ...
    performPurchase() {
        this.message = '';
        this.buttons = true;
        this._storeServiceMayoristas.updateCart(this.cartContent, this.idUser);
        this._storeServiceMayoristas.performPurchase(this.cartContent, this.idUser)
            .then((data: any) => {
                if (data.state) {
                    this.cartContent = [];
                    this.totalUnits = 0;
    ...
    canceledPurchase() {
        ...
        this.totalUnits = 0;
        this.totalPurchase = 0;
        this._productService.deleteCartMayorista(this.idUser)
            .subscribe(data => {
